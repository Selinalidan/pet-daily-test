...
export function initialState() {
  return {version:2, settings:{name:'茉莉', exam:'2026-10-17', effects:true, rate:0.85, readingRate:0.45, testDate:'2026-09-11'}, learned:{}, attempts:[], sessions:{}, completed:{}};
}
export function validState(s) {
  ...
  return s?.version === 2 && object(s.settings) && typeof s.settings.name === 'string' && s.settings.name.length <= 40 &&
    ...
    (!s.settings.readingRate || typeof s.settings.readingRate === 'number' && s.settings.readingRate >= 0.4 && s.settings.readingRate <= 0.8) &&
